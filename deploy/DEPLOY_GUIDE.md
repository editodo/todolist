...
### 1.3 백엔드(Server) 배포
1. 프로젝트의 `server` 폴더와 `ecosystem.config.js` 파일을 서버의 적절한 위치(예: `/home/damiftp/www/editodo`)로 업로드합니다.
2. 패키지 의존성을 설치합니다.
   ```bash
   cd /home/damiftp/www/editodo/server
...
1. 로컬 컴퓨터에서 웹용 정적 파일을 빌드합니다.
   ...
   npx expo export:web
   ```
2. 생성된 `web-build` 폴더 전체를 서버의 `/home/damiftp/www/editodo/web-build` 경로로 업로드합니다.
